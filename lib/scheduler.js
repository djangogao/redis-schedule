...
Scheduler.prototype.setRedisEvents = function () {
    ...
    this.enableExpiryNotifications();
...
Scheduler.prototype.schedule = function (options, cb) {
    ...
    if (validations.error) {
        ...
        if (options.expire || options.cronTime) {
            var self = this;
            if(options.expire) {
                this.clients.scheduler.exists(options.key, function (err, exists) {
                    if (exists) {
                        self.clients.scheduler.pexpire(options.key, self.getMillis(options.expire), cb);
                    } else {
                        self.clients.scheduler.set(options.key, '', 'PX', self.getMillis(options.expire), cb);
                    }
                });
            } else {
                var cronTime = cronParser.parseExpression(options.cronTime);
                var next = cronTime.next();
                while(next.getTime() <= new Date().getTime()) {
                    //console.log('Add Next: original: ' + next);
                    next = cronTime.next();
                    //console.log('Add Next: changed: ' + next);
                }
                //console.log(' Expire at: '+ next.toString());
                this.clients.scheduler.exists(options.key, function (err, exists) {
                    if (exists) {
                        self.clients.scheduler.pexpire(options.key, self.getMillis(next), cb);
                    } else {
                        self.clients.scheduler.set(options.key, '', 'PX', self.getMillis(next), cb);
                    }
                });
            }
        }
...
Scheduler.prototype.getMillis = function (expiration) {
    if (expiration instanceof Date || expiration.constructor.name === 'CronDate') {
        var now = new Date().getTime();
        expiration = expiration.getTime() - now;
    }

    //console.log('Expire remaining '+ expiration);
    return expiration;
};
...
Scheduler.prototype.enableExpiryNotifications = function () {
    this.clients.scheduler.config('SET', 'notify-keyspace-events', 'Ex');
};
